...
d3.csv("pivot_rating_c.csv", function (error, data) {
    ...
    d3.csv("pivot_counts.csv", function (error, relData) {
        ...
        d3.csv("drop_price.csv", function (error, neg) {

            var fil = []

            neg.forEach(function (d){
                 fil.push(d.business_name)
            });

            var fil_unique = fil.filter(function(item, i, ar){ return ar.indexOf(item) === i; });

            var temp_data = []

            data.forEach(function (d){
                if(fil_unique.indexOf(d.business_name) != -1){
                    temp_data.push(d)
                }
            });
            ...
            var legsymbols = legend.selectAll(".legsymbols")
                ...
                .attr("transform", function (d, i) {
                    return "translate(" + (65 + legwidths[i] ) + ",0)";
                });
            ...
            legsymbols.append("circle")
                ...
                .attr("cy", function (d, i) {
                    return (legendspace + 2 * rmax) - (rmax / ((-1) * ((i + 1) - 7)));
                })
            ...
            var legsymbols = legends.selectAll(".legsymbols")
                ...
                .attr("class", "legsymbols")
                .attr("transform", function (d, i) {
                    return "translate(" + (175 + legwidths[i] ) + ",0)";
                });
